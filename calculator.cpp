...
int main()
{
    ...
    while (!inFile.eof())
    {
        ...
        {
            cout << "Subtraction is not done...." << endl;
...
{ // this function makes numbers in order.
    ...
    {
        if (travL != nullptr && travR == nullptr)
        {
            total->data = travL->data;
            travL = travL->next;
            if (travL != nullptr)
            {
                dig = total;
        ...
        else if (travL == nullptr && travR != nullptr)
        {
            total->data = travR->data;
            travR = travR->next;
            if (travR != nullptr)
            {
                dig = total;
        ...
        else
        {
            int carry = 0;
            string conv;
            total->data += travL->data + travR->data;
            travL = travL->next;
            travR = travR->next;
            if (total->data > 9)
            {
                conv = to_string(total->data);
                carry = int(conv[0] - '0');
                total->data = total->data - (int(conv[0] - '0') * 10);
            }
            dig = total;
            if (travL != nullptr && travR != nullptr)
            {
